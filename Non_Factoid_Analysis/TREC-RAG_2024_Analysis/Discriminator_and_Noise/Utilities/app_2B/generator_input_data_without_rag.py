...
for i in query_id_set:
    ...
    if len(disc_set)<disc_count:
        continue

    if len(gold_set)<gold_count:
        continue

    output_temp={'doc_id_discriminator':[], 'doc_id_gold':[], 'query_id':i}

    disc_counter=0
    while len(output_temp['doc_id_discriminator'])<disc_count:
        output_temp['doc_id_discriminator'].append(disc_set[disc_counter])
        disc_counter+=1

    gold_counter=0
    while len(output_temp['doc_id_gold'])<gold_count:
...
with open(rf'/home/irlab/sagnik/Non_Factoid_Analysis/TREC-RAG_2024_Analysis/Discriminator_and_Noise/Data/{method}/generator_input_data_id_gold_fixed_{gold_count}_app2B.jsonl','w') as f:
        for i in output:
